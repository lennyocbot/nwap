...
export default function ReminderWatcher() {
  ...
  useEffect(() => {
    if (!state.settings.reminders?.enabled && !state.settings.notifications) return
    if (!('Notification' in window) || Notification.permission !== 'granted') return

    const now = Date.now()
    if (state.settings.reminders?.assignments !== false) {
      state.assignments
    ...
    if (state.settings.reminders?.flashcards !== false) {
  ...
  }, [state.assignments, state.flashcards, state.settings.reminders, state.settings.notifications])

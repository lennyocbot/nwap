...
function viewRace(root) {
  ...
  const medStop = median(greenStops.map(st => st.dur));
  const bestGreen = greenStops[0];
  ...
  w3.innerHTML = `<table class="t"><thead><tr><th>Driver</th><th class="r">Lap</th><th>Change</th><th class="r">Pit lane</th><th class="r">vs median</th></tr></thead><tbody>` +
    stops.map(st => `<tr><td>${drvCell(st.d)}</td><td class="r num">${st.l.lap}${st.flag ? ` <span class="tag" style="background:var(--band-sc);color:var(--yellow)">${st.flag}</span>` : ""}</td>
      <td>${st.from ? cmpDot(st.from) : "?"} → ${st.to ? cmpDot(st.to) : "?"}</td>
      <td class="r num ${st === bestGreen ? "best" : ""}">${st.dur ? fmtSec(st.dur, 1) + "s" : "—"}</td>
      <td class="r num" style="color:${st.dur && medStop && st.dur - medStop > 1500 && !st.flag ? "var(--red)" : "var(--ink3)"}">${st.dur && medStop && !st.flag ? fmtDelta(st.dur - medStop, 1) : ""}</td></tr>`).join("") + "</tbody></table>";
  if (medStop) c3.insertAdjacentHTML("beforeend", `<p class="note">Median green-flag pit-lane time: <b class="num">${fmtSec(medStop, 1)}s</b> · ${stops.length} stops${stops.length - greenStops.length ? ` (${stops.length - greenStops.length} under SC/VSC/red — cheaper by nature, excluded from comparisons)` : ""}</p>`);

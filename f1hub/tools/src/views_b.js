...
function viewLongRuns(root) {
  ...
  for (const t of stintsOf(s)) {
    ...
    const d = HUB.driver(t.drv, sid);
    if (!d) continue;
    // deleted laps stay in: track-limit deletions are still real race pace
    const seq = [...t.laps].sort((a, b) => a.lap - b.lap);
    const valid = seq.filter(l => l.t != null && !l.in && !l.out);
    if (valid.length < 4) continue;
    const med = median(valid.map(l => l.t));
    const keep = l => l.t != null && !l.in && !l.out && l.t < med * 1.035;
    // a run must be CONSECUTIVE laps — an untimed or off-pace lap ends it, so
    // quali-sim push laps sharing the stint can't pollute the average; one
    // timed slow lap (traffic) is bridged when pace both sides agrees to 2%
    const segs = [];
    let cur = [];
    seq.forEach((l, i) => {
      if (keep(l)) { cur.push(l); return; }
      const prev = cur.at(-1), next = seq[i + 1];
      if (prev && next && l.t != null && keep(next) && Math.abs(next.t - prev.t) < Math.min(next.t, prev.t) * 0.02) return;
      if (cur.length) segs.push(cur);
      cur = [];
    });
    if (cur.length) segs.push(cur);
    segs.filter(sg => sg.length >= 4).forEach((run, si) => {
      const fit = linfit(run.map((l, i) => [i, l.t]));
      runs.push({ t, d, run, med: median(run.map(l => l.t)), best: Math.min(...run.map(l => l.t)), fit, key: t.drv + "#" + t.stint + (si ? "#" + si : "") });
    });
  }
  ...
  if (!runs.length) { root.insertAdjacentHTML("beforeend", `<div class="empty">No run of 4+ consecutive representative laps from the selected drivers — add drivers above, or nobody did race sims here.</div>`); return; }
  ...
  for (const r of runs) {
    const span = r.t.laps.filter(l => l.lap >= r.run[0].lap && l.lap <= r.run.at(-1).lap).length;
    r.pushCool = span > 0 && r.run.length / span < 0.62;   // lots of stripped laps = push-cool program
    ...
    r.repr = mark(r) === "out" ? false
      : mark(r) === "in" ? !!r.fit && r.run.length >= 4
  ...
  const c = card(root, "Long runs", `runs of 4+ consecutive laps in ${SNAMES[sid]} · out/in, push and cool-down laps stripped · fuel loads unknown`);
  ...
  // the sheet mirrors the runs you've selected in the chips above (same as the
  // comparison chart); falls back to all runs if nothing is selected
  lrSheet(root, s, sel.length ? sel : runs);
}
...
function lrSheet(root, s, runs) {
  ...
  for (const c of cols) {
    ...
    c.cells = laps.map(l => ({ t: l.t, x: !inRun.has(l.lap) }));
  ...
  const card2 = card(root, "Long run sheet", "lap-by-lap, X = stripped (push / cool-down / traffic) · sorted by average pace");
